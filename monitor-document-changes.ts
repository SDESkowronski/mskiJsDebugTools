 ...
 *
 * ORIGINAL AUTHOR: Michael Skowronski while at Microsoft
 * BROUGHT INTO PROJECT BY: Michael Skowronski while a Scorpion
 *
...
// To use this in your .spec file...
// import 'c:/mski/monitor-document-changes.js';

import * as helpers from './helpers.ms';
...
(function () {
  ...
  function Stop() {
    StopLookingForChanges = true;
  }

  function Initialize() {
    helpers.ConLog(`MonitorDocumentChanges.initialize()`, `Start`);

    Cypress.Commands.add('ConLog', (funcName: string, message: string) => {
    ...
    Cypress.Commands.add('Get', (selector, options) => {
      helpers.ConLog(
        `cy.Get(${selector})`,
        `Start - Last DOM change was ${MillisecondsSinceLastChange()} milliseconds ago`
      );
    ...
    Cypress.Commands.add(
      ...
      (subject, selector, textItShouldNotContain, expectFailure = false) => {
    ...
    function DoesNotContain(subject, selector, textItShouldNotContain, expectFailure, exactMatch) {
      ...
      waitForStableDOM().then(() => {
        ...
        if (elements.length > 0) {
          if (!textItShouldNotContain) {
            foundCount = elements.length;
          } else {
            for (let i = 0; i < elements.length; i++) {
              let elementText = helpers.TextContentWithoutNewlines(elements[i]);
              if (
                (exactMatch && elementText === textItShouldNotContain) ||
                (!exactMatch && elementText.includes(textItShouldNotContain))
              ) {
                helpers.ConLog(
                  funcName,
                  `Found "${textItShouldNotContain}" in this element >>> ${elements[i].outerHTML}`
                );
                foundCount++;
              }
            }
        ...
        if (foundCount > 0) {
          ...
          throw new Error(
            `selector: "${selector}" & textItShouldNotContain: "${textItShouldNotContain}" was expected to be missing from the DOM, instead we found ${foundCount} instances of it.`
          );
        }
    ...
    function waitForStableDOM() {
      return cy
        .wrap('Waiting for Milliseconds Since Last Change to be greater than 700', { timeout: 60000 })
        .should(() => {
          assert.isBelow(700, MillisecondsSinceLastChange());
        });
    }
    ...
    Cypress.Commands.add('WaitTillNChangesOccur', { prevSubject: 'optional' }, (elements, changeCount) => {
      helpers.ConLog(
        `cy.WaitTillNChangesOccur(${changeCount})`,
        `Start - Last DOM change was ${MillisecondsSinceLastChange()} milliseconds ago`
      );
    ...
    Cypress.Commands.add('RunAndExpectDomChange', (func) => {
      ...
      helpers.ConLog(
        `cy.RunAndExpectDomChange()`,
        `done - Last DOM change was ${MillisecondsSinceLastChange()} milliseconds ago`
      );
      lastChangeTime = new Date().getTime();
    });

    Cypress.Commands.add('DumpHtmlOnDomChange', (boolValue) => {
      dumpHtml = boolValue;
    });

    var windowLoadStatus: string = 'waiting';
    ...
    Cypress.on('window:load', () => {
      ...
      lastChangeTime = new Date().getTime();
      windowLoadStatus = 'complete';
    });

    Cypress.Commands.add('WatchForWindowReload', () => {
      windowLoadStatus = 'waiting';
    });

    Cypress.Commands.add('WaitForWindowLoadComplete', () => {
      helpers.ConLog(
        `cy.WaitForWindowLoadComplete()`,
        `Start - Last DOM change was ${MillisecondsSinceLastChange()} milliseconds ago`
      );
      return cy.wrap('Waiting for a window load to complete...', { timeout: 60000 }).should(() => {
        assert.notEqual('waiting', windowLoadStatus);
      });
    });
    ...
    Cypress.on('url:changed', (newUrl) => {
      helpers.ConLog(
        `url:changed`,
        `New URL ${newUrl} - MillisecondsSinceLastChange: ${MillisecondsSinceLastChange()}`
      );
      lastChangeTime = new Date().getTime();
    });
    ...
    setTimeout(() => {
      LookForChange(true);
    }, 50); // This will repeat until Stop is called.
  ...
  function LookForChange(loop) {
    ...
    if (loop)
      setTimeout(() => {
        LookForChange(true);
      }, 50); // Repeat this same function 50ms later
  }

  function MonitorSpinner() {
    const funcName = `MonitorDocumentChanges.MonitorSpinner()`;

    const spinnerTexts = ['class="loader ng-star-inserted"', 'scorpion-spinner'];
    ...
    for (let i = 0; i < spinnerTexts.length; i++) {
      if (lastHtml.includes(spinnerTexts[i])) {
        ...
        if (spinnerTexts[i] != currentSpinnerText) {
          if (dumpHtml) {
            helpers.ConLog(funcName, `Start - ${spinnerTexts[i]} - current HTML:\n${lastHtml}`);
          }
  ...
  function SetExpectingSpinner(value) {
    ...
    helpers.ConLog(
      `MonitorDocumentChanges.SetExpectingSpinner()`,
      `Value Changed from ${expectingSpinner} to ${value}`
    );
